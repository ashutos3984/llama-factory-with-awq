...
from transformers import BitsAndBytesConfig, EetqConfig, GPTQConfig, HqqConfig, AwqConfig
...
def configure_quantization(
    ...
    if getattr(config, "quantization_config", None):  # ptq
        ...
        elif model_args.quantization_method == QuantizationMethod.AWQ.value:
            if model_args.quantization_bit not in [4, 8]:
                raise ValueError("AWQ only accepts 4-bit or 8-bit quantization.")

            if is_deepspeed_zero3_enabled() or is_fsdp_enabled():
                raise ValueError("AWQ quantization is incompatible with DeepSpeed ZeRO-3 or FSDP.")

            require_version("autoawq", "To fix: pip install autoawq")
            init_kwargs["quantization_config"] = AwqConfig(
                bits=model_args.quantization_bit,
                fuse_max_seq_len=512,  # Adjust based on your needs
                do_fuse=True,  # Adjust based on your needs
            )
            logger.info("Quantizing model to {} bit with AWQ.".format(model_args.quantization_bit))
